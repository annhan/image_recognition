...
class image_recognition:
    ...
    def detectObjects(self, image, bg_img,externalContours=True):
        ...
        if externalContours==True:
            _ , contours_detected, hierarchy  = cv2.findContours(diff, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
        else:
            _ , contours_detected, hierarchy = cv2.findContours(diff, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
